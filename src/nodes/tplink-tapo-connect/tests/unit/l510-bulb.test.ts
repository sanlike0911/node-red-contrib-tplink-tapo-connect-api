/**
 * L510 Smart Bulb Unit Tests
 * Tests the L510 class implementation (dimmable white light only)
 */

import { L510Bulb } from '../../src/devices/bulbs/l510-bulb';
...
describe('L510Bulb Unit Tests', () => {
  ...
  beforeEach(() => {
    ...
    bulb = new L510Bulb('192.168.1.100', mockCredentials);
  });

  describe('Class Instantiation', () => {
    test('should create L510Bulb instance', () => {
      expect(bulb).toBeInstanceOf(L510Bulb);
      expect(bulb).toBeDefined();
    });

    test('should have correct device model', () => {
      expect((bulb as any).getDeviceModel()).toBe('L510');
    });
  });

  describe('Device Capabilities', () => {
    test('should have brightness but no color capabilities', () => {
  ...
  describe('Brightness Control', () => {
    ...
    test('should get current brightness', async () => {
      const mockSendRequest = jest.fn().mockResolvedValue({
        error_code: 0,
        result: { brightness: 65, device_on: true }
      });
      (bulb as any).sendRequest = mockSendRequest;
